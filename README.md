...
- Redis instance for server-side OAuth2 security
...
## Self Hosting
If you wish to self host an instance, it's recommended to use `docker-compose`. The included [docker-compose.yml](docker-compose.yml) will build the project, startup the required services (redis, bot, webapp), and expose the webapp at `localhost:5000`. You will need to provide a `.env` with the required API keys. See [keys.py](keys.py) for details.
