...
async def on_tree_error(interaction: discord.Interaction, error: app_commands.AppCommandError):
    if isinstance(error, app_commands.MissingRole):
        return await interaction.response.send_message(f"You require the {POLYCULE_ADMIN_ROLE} role to run this command.",
                                                       ephemeral=True)
    else:
        raise error
...
async def add_partner(interaction: discord.Interaction,
...
async def _add_partner(interaction: discord.Interaction,
                       relationship_type: RelationshipType,
                       partner: discord.Member = None,
                       partner_name: str = None):
    if partner is not None and partner_name is not None:
        await interaction.response.send_message("❌ ERROR: Please use either partner or partner_name and not both ❌",
                                                ephemeral=True)
    elif partner is None and partner_name is None:
        await interaction.response.send_message("❌ ERROR: Please use either partner or partner_name ❌",
                                                ephemeral=True)
    else:
        try:
            if partner:
                polycules.get(interaction.guild_id).add_self_relationship(interaction.user.id, partner.id,
                                                                          partner.display_name, relationship_type)
            else:
                polycules.get(interaction.guild_id).add_self_relationship(interaction.user.id, None,
                                                                          partner_name, relationship_type)

            await interaction.response.send_message(f"✅ I added {partner} as your partner! ✅")
        except RegistrationError:
            await interaction.response.send_message("❌ ERROR: Please use /register before adding a partner ❌",
                                                    ephemeral=True)
...
@app_commands.describe(person1_discord="The first person's Discord name",
                       person1_name="The first person's name (only use if they aren't on this server and "
                                    "not in combination with person1_discord)",
                       person2_discord="The second person's Discord name",
                       person2_name="The second person's name (only use if they aren't on this server and "
                                    "not in combination with person2_discord)",
                       relationship_type="What type of relationship is this?")
@app_commands.checks.has_role(POLYCULE_ADMIN_ROLE)
async def add_others_relationship(interaction: discord.Interaction,
                                  relationship_type: RelationshipType,
                                  person1_discord: discord.Member = None,
                                  person1_name: str = None,
                                  person2_discord: discord.Member = None,
                                  person2_name: str = None):

    await _add_others_relationship(interaction, relationship_type, person1_discord, person1_name, person2_discord,
                                   person2_name)


async def _add_others_relationship(interaction: discord.Interaction,
                                   relationship_type: RelationshipType,
                                   person1_discord: discord.Member = None,
                                   person1_name: str = None,
                                   person2_discord: discord.Member = None,
    ...
    if person1_discord is not None and person1_name is not None:
        await interaction.response.send_message("❌ ERROR: Please use either person1_discord or person1_name and not both ❌",
                                                ephemeral=True)
    elif person2_discord is not None and person2_name is not None:
        await interaction.response.send_message("❌ ERROR: Please use either person2_discord or person2_name and not both ❌",
                                                ephemeral=True)
    elif person1_discord is None and person1_name is None:
        await interaction.response.send_message("❌ ERROR: Please use either person1_discord or person1_name ❌",
                                                ephemeral=True)
    elif person2_discord is None and person2_name is None:
        await interaction.response.send_message("❌ ERROR: Please use either person2_discord or person2_name ❌",
                                                ephemeral=True)
    else:
        person1_id = None
        person2_id = None
        ...
        polycules.get(interaction.guild_id).add_others_relationship(person1_id, person1_name, person2_id, person2_name,
                                                                    relationship_type)
