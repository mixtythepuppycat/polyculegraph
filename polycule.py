...
class Polycule:
    ...
    def add_others_relationship(self, person1_id, person1_name: str, person2_id, person2_name: str,
    ...
    def _add_node(self, id, display_name: str, pronouns: str = "", type: str = "", claimed: bool = False,
        ...
        if not self.G.has_node(id) or force_update:
            self.G.add_node(id, display_name=display_name, click=f"Pronouns: {pronouns}<br/>Critter Type: {type}",
                            claimed=claimed, unique_id=unique_id)

        return id

    def _add_edge(self, person1_id, person2_id, type: RelationshipType):
        self.G.add_edge(person1_id, person2_id, relationship=type.name, color=type.value,
                        click=f"Relationship Type: {type.name}", size=5)
